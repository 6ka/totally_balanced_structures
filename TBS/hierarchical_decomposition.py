...
def hierarchical_height_from_lattice(dismantable_lattice):
    ...
    Args:
        dismantable_lattice (graph.Graph):

    Returns:
        A :class:`dict` whose key are the hierarchical level (0 is the first hierarchy) and value the associated element
        of the lattice.
    ...
    sup_irreducibles = set(lattice.sup_irreducible(dismantable_lattice))
...
def unique_successor_decomposition(dismantable_lattice):
    ...
    maxima = [lattice.get_top(cover_graph)]
    current_height = 0
