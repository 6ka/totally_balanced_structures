...
class TestDissImpexport(unittest.TestCase):
    ...
    def test_type(self):
        ...
        self.assertRaises(ValueError, DLC.diss.file_io.load, f)
        ...
        self.assertRaises(TypeError, DLC.diss.file_io.load, f, "a crazy type")
        self.assertRaises(ValueError, DLC.diss.file_io.load, f)
        f.close()
        dprim = Diss([0])
        ...
        f = open("../resources/test_load_1.mat")
        d = DLC.diss.file_io.load(f, number=False)
        f.close()
        dprim = self.d.copy()
        for x in dprim:
            for y in dprim:
