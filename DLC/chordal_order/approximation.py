
from DLC.chordal_order.order import from_diss_approximate_order

def chordal_diss_from_fixed_order(diss, order):
    """modifiy diss to be chordal according to order"""

    index_order = [diss.vertex_index[x] for x in order]
    for i in range(len(order)):
        x = index_order[i]
        for j in range(i + 1, len(order)):
            y = index_order[j]
            for k in range(j + 1, len(order)):
                z = index_order[k]
                if diss.get_by_pos(y, z) > max(diss.get_by_pos(x, y), diss.get_by_pos(x, z)):
                    diss.set_by_pos(y, z, max(diss(x, y), diss(x, z)))


def chordal_diss_approximation(diss):
    """modifiy diss to be chordal. Finds a possible order"""
    possible_order = from_diss_approximate_order(diss)
    chordal_diss_from_fixed_order(diss, possible_order)